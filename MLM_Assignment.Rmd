...
- __Linearity Assumption__ - Points spread roughly evenly above and below the 0 line with some visible lines assessed to be as a result of the data simulation process, therefore the assumption of a linear relationship between predictor variables and the response variable plausibly holds. 
...
final_model_randomised = lmer(formula = Response ~ 
                   1 + Treatment + Time + Treatment:Time + Experience + Gender + Size + 
                   (1 | Hospital) + 
                   (1 | Hospital : Nurse), 
                   data = mst_data_randomised)

residuals_1_to_n_minus_1 = residuals(final_model_randomised)[1:(length(residuals(final_model_randomised)) - 1)]

residuals_2_to_n = residuals(final_model_randomised)[2:length(residuals(final_model_randomised))]
...
ggplot(mst_data_visualisation_subset, aes(Time, Response)) +
  ...
  geom_point(aes(Time, Response, colour = Nurse), size = 1.5, alpha = 0.5, position = position_jitter(width = 0.2, height = 0.2)) +
