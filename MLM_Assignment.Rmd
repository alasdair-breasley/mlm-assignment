...
A naive analysis of the above histograms indicates that there is an initial treatment effect and that this treament effect reduces over time. The Stress Score distributions of the control and treatment groups start away from each other with little overlap, and with each time step the treatment group distribution moves right along the Stress Score axis converging towards the control group distribution i.e. the treatment group Stress Scores increase over time post treatment with the control group remianing similar. 
...
A naive analysis of the above plot indicates that the median Stress Score in the treament group is ~4.5 lower than the control group one month after treatment, with this reducing to ~4 and ~2.5 lower after two and three months respectively. As with the histograms, the treatment group Stress Scores increase over time post treatment converging towards the control group at an increase in median Stress Score of ~1 per month. 
...
- __Linearity Assumption__ - Points spread roughly evenly above and below the 0 line with some lines assessed to be as a result of the data simulation process, therefore the assumption of a linear relationship between predictor variables and the response variable plausibly holds. 
...
There appears to be a small positive association between adjascent residuals so residuals were randomised and reanalysed. 
...
indices = sample(seq(1, nrow(mst_data), by = 1), replace = FALSE, prob = rep(1 / nrow(mst_data), nrow(mst_data)))
